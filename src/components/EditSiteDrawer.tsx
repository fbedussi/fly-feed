...
const EditSiteDrawer: Component = () => {
  ...
  const [deletedSite, setDeletedSite] = createSignal<(SiteDb & { categoryId?: string }) | null>(
    null,
  )
  ...
  const updateSite = () => {
    ...
    const isNewSite = !subscriptionsQuery.data.categories
      .find(({ id }) => id === siteToEditData.categoryId)
      ?.sites.some(({ id }) => id === siteToEditData.id)

    const updatedData = isNewSite
      ? {
          ...subscriptionsQuery.data,
          categories: subscriptionsQuery.data.categories.map(category =>
            category.id === siteToEditData.categoryId
              ? {
                  ...category,
  ...
  return (
    <>
      <TopDrawer open={!!siteToEdit()} onClose={() => setSiteToEdit(null)}>
        <div class={styles.topDrawerInner}>
          ...
          >
            ...
            <TextField
              label="rss link"
              value={siteToEdit()?.xmlUrl || ''}
              onChange={e => {
                const siteToEditData = siteToEdit()
                siteToEditData &&
                  setSiteToEdit({ ...siteToEditData, xmlUrl: e.currentTarget.value })
              }}
            />
            <TextField
              label="web link"
              value={siteToEdit()?.htmlUrl || ''}
              onChange={e => {
                const siteToEditData = siteToEdit()
                siteToEditData &&
                  setSiteToEdit({ ...siteToEditData, htmlUrl: e.currentTarget.value })
              }}
            />
            <div class={styles.muteAndDelete}>
              <FormGroup>
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={siteToEdit()?.muted || false}
                      onChange={e => {
                        const siteToEditData = siteToEdit()
                        siteToEditData &&
                          setSiteToEdit({
            ...
            <div class={styles.buttons}>
              <Button variant="text" onClick={() => setSiteToEdit(null)}>
                cancel
              </Button>
